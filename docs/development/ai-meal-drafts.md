...
- AI-suggested new dishes carry a full editable dish draft: category, tags, base servings, ingredients with quantity/unit/category and instructions.
- Suggested new dishes can be removed from the draft or replaced one at a time without resetting the rest of the form.
- Confirming the meal writes suggested new dishes through the normal dish creation service before creating the meal plan, so the generated shopping list can use their ingredients immediately.
- First-use meal creation now saves as `待确认`, matching the share-feedback path instead of closing feedback before the share link is opened.
...
- Cost and reliability limits: max 2 attempts, 12s per attempt and 2400 output tokens.
...
- Suggested new dish categories, tags, ingredient categories and ingredient units are normalized against `src/lib/domain/food-options.ts`.
