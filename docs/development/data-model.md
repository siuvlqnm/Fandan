...
- `created_by_user_id`、`updated_by_user_id` 记录家庭成员操作归属；历史数据允许为空。
...
- `created_by_user_id`、`updated_by_user_id` 记录创建和最近处理成员；状态切换也会更新最近处理成员。
...
- `created_by_user_id`、`updated_by_user_id` 记录条目创建和最近处理成员。
...
- 保存勾选状态，便于移动端买菜使用。
- `created_by_user_id`、`updated_by_user_id` 记录购物项创建和最近编辑成员。
- `checked_by_user_id`、`checked_at` 记录最近标记已买的成员和时间；取消勾选时清空。
...
- `*_created_by_idx`、`*_updated_by_idx` and `shopping_list_items_checked_by_idx`: Phase 10 operation ownership display and future family activity queries.
