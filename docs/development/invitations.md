...
Acceptance is idempotent for the user who already accepted the link. A different user receives a conflict after the invitation is accepted. Owners, active members and historical membership rows cannot create a duplicate user-space pair through an invitation.
