import { apiError } from '$lib/server/api/errors';
import { apiRoute } from '$lib/server/api/handler';
import { apiOk } from '$lib/server/api/response';
import { parseJsonBody } from '$lib/server/api/validation';
import { requireUserSpace } from '$lib/server/context';
import { createShoppingListItem, createShoppingListItemSchema } from '$lib/server/shopping-lists';

const getShoppingListId = (id: string | undefined) => {
	if (!id) {
		throw apiError('BAD_REQUEST', 'Shopping list id is required');
	}

	return id;
};

export const POST = apiRoute(async (event) => {
	const context = await requireUserSpace(event);
	const body = await parseJsonBody(event, createShoppingListItemSchema);
	const shoppingList = await createShoppingListItem(context, getShoppingListId(event.params.id), body);

	return apiOk({ shoppingList }, { status: 201 });
});
