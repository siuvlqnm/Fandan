...
import { createShareFeedback, createShareFeedbackSchema } from '$lib/server/share-links';
...
export const POST = apiRoute(async (event) => {
	...
	const body = await parseJsonBody(event, createShareFeedbackSchema);
	const feedback = await createShareFeedback(context, getToken(event.params.token), body);
