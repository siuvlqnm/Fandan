...
<main class="mx-auto flex min-h-svh max-w-md flex-col justify-center gap-5 px-4 py-8 md:max-w-2xl">
	{#if !data.invitation || !data.space}
		<section class="app-panel space-y-5 p-5 md:p-8">
			...
			<div class="space-y-2"><p class="app-chip bg-muted text-muted-foreground">家庭空间邀请</p><h1 class="text-2xl font-semibold">邀请链接不可用</h1><p class="text-sm leading-6 text-muted-foreground">{unavailableMessage}</p></div>
		</section>
	{:else if joined}
		<section class="app-panel space-y-5 p-5 text-center md:p-8">
			...
			<div class="space-y-2"><p class="app-chip bg-secondary text-primary">加入成功</p><h1 class="text-2xl font-semibold">已经加入「{data.space.name}」</h1><p class="text-sm leading-6 text-muted-foreground">当前工作区已经切换，可以开始查看家人共享的菜品、饭单和购物清单。</p></div>
	...
	{:else if data.membership === 'owner'}
		<section class="app-panel space-y-5 p-5 md:p-8">
			...
			<div class="space-y-2"><h1 class="text-2xl font-semibold">这是你自己的空间</h1><p class="text-sm leading-6 text-muted-foreground">空间所有者不能接受自己创建的邀请。请把链接发给要加入的家人。</p></div>
			<Button href="/app/invitations" class="h-12 w-full rounded-2xl">返回邀请管理</Button>
		</section>
	{:else if data.invitation.state !== 'pending'}
		<section class="app-panel space-y-5 p-5 md:p-8">
			<span class="flex size-12 items-center justify-center rounded-2xl bg-destructive/10 text-destructive"><AlertCircle class="size-6" /></span>
			<div class="space-y-2"><p class="app-chip bg-muted text-muted-foreground">家庭空间邀请</p><h1 class="text-2xl font-semibold">暂时无法加入</h1><p class="text-sm leading-6 text-muted-foreground">{unavailableMessage}</p></div>
		</section>
	{:else}
		<section class="app-panel overflow-hidden">
			<div class="space-y-5 bg-secondary/60 p-5 md:p-8">
				...
				<div class="space-y-3"><p class="app-chip bg-white text-primary">家庭空间邀请</p><h1 class="text-3xl font-semibold leading-tight">加入「{data.space.name}」</h1><p class="text-sm leading-6 text-muted-foreground">加入后，家里的菜品、饭单、确认结果和购物清单会在同一个空间里同步。</p></div>
